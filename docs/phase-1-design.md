...
플러그인은 accounts의 활성 계정·순서를 고정하고 A 수집→임시 JSONL 종료·Excel 원본 확정·기준 표시→수집 계정 간 휴식→B 수집을 진행한다. 요청마다 계정당 새 미디어 원글 최대 2건이 기본이며 사용자가 이번 요청에 다른 건수를 지정할 수 있다. 같은 날 여러 번 요청해도 이전 기준 ID까지 새 글을 확인한다. 수집 중에는 JSONL로 누적하고 계정 종료 시 후처리한다. 플러그인은 다운로드를 시작하지 않으며 state.db·다운로드 중단 상태를 읽거나 쓰지 않는다.
...
전체 결과 검증 후 메모리 모델을 교체한다. 실패하면 직전 정상 화면을 유지할 수 있으나 `원본 변경/확인 필요`로 표시하고 해당 원본의 새 다운로드는 시작하지 않는다. 같은 파일명도 내용 변경을 확인한다. 확정되지 않은 running 참조는 보류하고 partial·충돌·주소 미확보는 실제 상태를 보존한다. 현재 다운로드 가능 여부는 게시글별 캡션·첨부 수집 상태와 실제 첨부·URL 연결로 판단한다. 상위 실행이 중단되거나 partial이어도 완전한 게시글은 계획·미리보기·명시적 재개 대상으로 유지한다. 초기 파일 하나 파일럿의 완료 실행 한정 조건을 현재 정책으로 적용하지 않는다.
...
시작 전에 게시글의 남은 첨부 전부에 유효한 원본 참조와 허용된 후보가 있는지 확인한다. 앱 시작·새로고침과 새 배치 전 cleanup-source는 source_not_ready·attachment_not_ready 중 작업 없는 불완전 수집 글만 post_deletions로 논리적으로 삭제하고 cleanedPosts를 안내한다. jobs·부분/다운로드 파일·AI 생성본·등록·편집이 하나라도 있으면 보호하며 충돌·기존 배치 오류를 정리로 우회하지 않는다. Excel·수집 기준·원본 해시를 유지하고 HTTP 요청·파일 삭제는 하지 않는다. 정상 게시글의 기존 download_exclusions는 해제하고 실제 불완전한 작업 없는 글만 삭제 기록으로 이관한다. 전역 중단 상태가 없을 때 현재 계정에서 확정한 대상 게시글만 등록일 오래된 순서로 묶는다. 등록일 미상은 최초 수집일로 보조 정렬하고 동률은 계정·문자열 ID로 고정한다. 활성 작업은 하나만 허용하고 같은 버튼을 두 번 눌러도 중복 요청하지 않는다. 정리한 불완전 글 수는 `cleanedPosts`, 정상으로 확인해 제외를 해제한 글 수는 `releasedPosts`로 구분한다.
...
| 상황                                   | 처리                                                                                                                                               |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| 시작 전 blob_unresolved / missing      | 두 준비 불가 판정 코드에 해당하는 작업 없는 불완전 수집 글만 앱 DB 논리 삭제. 기존 작업이 있으면 보존하며 작업 중 새로 발견되면 전체 중단. HTTP 요청 없음 |
...
파일 상태는 `미다운로드 / 대기 / 다운로드 중 / 확인된 파일 저장 / 실패 / 중단 / 파일 없음`으로 구분한다. 오류 게시글과 앱 다운로드 실행은 `미완료 · 오류로 중단`, 뒤의 계정·게시글은 다운로드 대기, 기존 완료 파일·게시글은 완료로 보존한다. 작업 없는 불완전 수집 글 정리는 정상 글의 다운로드 실패와 구분한다. 기존 작업이 있거나 원본 충돌 등으로 정리할 수 없는 보류는 별도 사유로 표시한다. 게시글 완료는 선택한 원본 버전에서 수집된 첨부 전부의 저장 완료이며 수집 상태 partial을 complete로 승격하지 않는다. 파일 저장 성공과 미리보기 지원 여부도 구분한다.
