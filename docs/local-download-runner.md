...
파일 하나의 순차 HTTPS GET, 허용 CDN·DNS·TLS 검증, 저장·파일 검증, 단일 SQLite 상태, 소비 기록, 첫 오류 전체 중단과 로컬 저장 복구를 포함한다. 기존 오프라인 테스트와 실제 이미지19개·영상1개 전체 저장 기록은 [진행 기록](progress.md)에 있다. Electron 연결은 [1D](phase-1d-download.md)에서 구현했고 배치 회차는 다음1E다.
